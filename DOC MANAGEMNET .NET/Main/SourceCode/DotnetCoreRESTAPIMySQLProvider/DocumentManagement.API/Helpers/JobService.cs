...
namespace DocumentManagement.API.Helpers
{
    public class JobService
    {
        ...
        public void StartScheduler()
        {
            ...
            //Daily Reminder
            RecurringJob.AddOrUpdate(() => DailyReminder(), "0 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //Weekly Reminder
            RecurringJob.AddOrUpdate(() => WeeklyReminder(), "10 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //Monthy Reminder
            RecurringJob.AddOrUpdate(() => MonthyReminder(), "20 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //Quarterly Reminder
            RecurringJob.AddOrUpdate(() => QuarterlyReminder(), "30 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //HalfYearly Reminder
            RecurringJob.AddOrUpdate(() => HalfYearlyReminder(), "40 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //Yearly Reminder                                                                                
            RecurringJob.AddOrUpdate(() => YearlyReminder(), "50 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //Customer Date
            RecurringJob.AddOrUpdate(() => CustomDateReminderSchedule(), "59 0 * * *", TimeZoneInfo.Local); // Every 24 hours

            //Reminder Scheduler To Send Email
            RecurringJob.AddOrUpdate(() => ReminderSchedule(), "* */10 * * *", TimeZoneInfo.Local); // Every 10 minutes

            //Send Email Scheduler To Send Email
            RecurringJob.AddOrUpdate(() => SendEmailSuppliersSchedule(), "* */15 * * *", TimeZoneInfo.Local); // Every 10 minutes
