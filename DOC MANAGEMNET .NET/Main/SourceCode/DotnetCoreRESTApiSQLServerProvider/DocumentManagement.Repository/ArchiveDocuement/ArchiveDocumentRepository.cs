...
{
    public class ArchiveDocumentRepository : GenericRepository<ArchiveDocument, DocumentContext>,
          IArchiveDocumentRepository
    {
        ...
        public ArchiveDocumentRepository(
        ...
        public async Task<ArchiveDocumentList> GetArchiveDocuments(DocumentResource documentResource)
        {
            var collectionBeforePaging = AllIncluding(c => c.User, cs => cs.Category);
            collectionBeforePaging =
               collectionBeforePaging.ApplySort(documentResource.OrderBy,
               _propertyMappingService.GetPropertyMapping<ArchiveDocumentDto, ArchiveDocument>());
            ...
            var documentList = new ArchiveDocumentList();
        ...
        public async Task<ArchiveDocumentList> GetArchiveDocumentsLibrary(string email, DocumentResource documentResource)
        {
            ...
            collectionBeforePaging = collectionBeforePaging
                .ApplySort(documentResource.OrderBy, _propertyMappingService.GetPropertyMapping<ArchiveDocumentDto, ArchiveDocument>());
        ...
        public async Task<ArchiveDocumentDto> GetArchiveDocumentById(Guid Id)
        {
            var today = DateTime.Now;
            var user = await _userRepository.AllIncluding(c => c.UserRoles).FirstOrDefaultAsync(c => c.Id == Guid.Parse(_userInfoToken.Id));
            var userRoles = user.UserRoles.Select(c => c.RoleId).ToList();
            ...
            var result = _mapper.Map<ArchiveDocumentDto>(document);
