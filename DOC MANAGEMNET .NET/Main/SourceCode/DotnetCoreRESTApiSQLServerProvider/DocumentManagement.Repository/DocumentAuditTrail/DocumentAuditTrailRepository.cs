...
namespace DocumentManagement.Repository
{
        ...
        IDocumentAuditTrailRepository
    {
        ...
        public async Task<DocumentAuditTrailList> GetRecentlyViewedDocuments(DocumentResource documentResource)
        {
            ...
            {
                collectionBeforePaging = collectionBeforePaging
                    .Where(c => c.CreatedBy.Equals(documentResource.Id));
            }*/

            var documentAuditTrailList = new DocumentAuditTrailList();
            var documentAuditTrailList2 = new DocumentAuditTrailList();
            documentAuditTrailList = await documentAuditTrailList.Create(
                collectionBeforePaging,
                documentResource.Skip,
                documentResource.PageSize
                );

            List<Guid> arrDocmentId = new List<Guid>();

            for(int i = 0; i < documentAuditTrailList.Count; i++)
            {
                var documentAuditTrail = documentAuditTrailList[i];
                if(!arrDocmentId.Contains(documentAuditTrail.DocumentId))
                {
                    arrDocmentId.Add(documentAuditTrail.DocumentId);
                    documentAuditTrailList2.Add(documentAuditTrail);
                } 
            }

            return documentAuditTrailList2;
        }

        public async Task<DocumentAuditTrailList> GetFavouirteDocuments(DocumentResource documentResource)
        {
            var collectionBeforePaging = AllIncluding(c => c.CreatedByUser, d => d.Document, c => c.Document.Category)
                .Where(e => (e.OperationName.Equals(DocumentOperation.Read) || e.OperationName.Equals(DocumentOperation.Download)));
            ...
            Dictionary<Guid, int> dicDocmentId = new Dictionary<Guid, int>();
            List<Guid> arrDocmentId = new List<Guid>();
            ...
            for (int i = 0; i < documentAuditTrailList.Count; i++)
            {
                ...
                {
                    arrDocmentId.Add(documentAuditTrail.DocumentId);
                    dicDocmentId.Add(documentAuditTrail.DocumentId, 1);
                } else
                {
                    var count = dicDocmentId[documentAuditTrail.DocumentId];
                    dicDocmentId[documentAuditTrail.DocumentId] = count + 1;
                    arrDocmentId.Add(documentAuditTrail.DocumentId);
                    if (count == 2)
                    {
                        documentAuditTrailList2.Add(documentAuditTrail);
